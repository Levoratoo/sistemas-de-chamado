...
class TicketMockupController extends Controller
{
    public function create(Request $request)
    {
        $areas = Area::where('active', true)->get();
        $categories = Category::where('active', true)->get();
        $requestType = RequestType::where('slug', 'mockup')->first();
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    'name' => 'Mock up',
                    'description' => 'Categoria para solicitações de mock up',
                    'active' => true,
                ]);
            }

            $slaData = $slaService->calculateSlaForTicket('mockup', 'medium', $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'title' => 'Mock up - ' . Sanitizer::sanitize($request->input('titulo')),
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Solicitação de Mock up criada com sucesso!');
    }
