...
class TicketSystemsController extends Controller
{
    public function create(Request $request)
    {
        ...
        $requestType = RequestType::where('slug', 'sistemas-programas')->first();
        
        return view('tickets.create-systems', compact('areas', 'categories', 'requestType'));
    }
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    ...
                    'description' => 'Categoria para sistemas e programas',
                    'active' => true,
                ]);
            }

            // Calcular SLA
            $slaData = $slaService->calculateSlaForTicket('sistemas-programas', $request->input('impacto', 'medium'), $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'status' => 'open',
                'priority' => $request->input('prioridade', $request->input('impacto', 'medium')),
                'request_type' => 'sistemas-programas',
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Chamado de Sistemas e Programas criado com sucesso!');
    }
