...
class TicketProvaContratualController extends Controller
{
    public function create(Request $request)
    {
        $areas = Area::where('active', true)->get();
        $categories = Category::where('active', true)->get();
        $requestType = RequestType::where('slug', 'prova-contratual')->first();
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    ...
                    'description' => 'Categoria para solicitações de Prova Contratual',
                    'active' => true,
                ]);
            }

            $slaData = $slaService->calculateSlaForTicket('prova-contratual', 'medium', $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'title' => 'Prova Contratual - ' . Sanitizer::sanitize($request->input('titulo')),
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Solicitação de Prova Contratual criada com sucesso!');
    }
