...
class TicketPuxadaCorController extends Controller
{
    public function create(Request $request)
    {
        $areas = Area::where('active', true)->get();
        $categories = Category::where('active', true)->get();
        $requestType = RequestType::where('slug', 'puxada-cor')->first();

        // Reutiliza o mesmo formulário do Mock up (mesmos campos/opções)
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    'name' => 'Puxada de Cor',
                    'description' => 'Categoria para solicitações de puxada de cor',
                    'active' => true,
                ]);
            }

            $slaData = $slaService->calculateSlaForTicket('puxada-cor', 'medium', $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'title' => 'Puxada de Cor - ' . Sanitizer::sanitize($request->input('titulo')),
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Solicitação de Puxada de Cor criada com sucesso!');
    }
