...
class TicketThreeDSiteController extends Controller
{
    public function create(Request $request)
    {
        $areas = Area::where('active', true)->get();
        $categories = Category::where('active', true)->get();
        $requestType = RequestType::where('slug', '3d-site')->first();

        // Reutiliza o mesmo formulário para manter os campos idênticos
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    'name' => '3D/Site',
                    'description' => 'Categoria para solicitações de 3D/Site',
                    'active' => true,
                ]);
            }

            $slaData = $slaService->calculateSlaForTicket('3d-site', 'medium', $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'title' => '3D/Site - ' . Sanitizer::sanitize($request->input('titulo')),
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Solicitação 3D/Site criada com sucesso!');
    }
