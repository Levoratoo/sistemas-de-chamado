...
class TicketMockupPrintedController extends Controller
{
    public function create(Request $request)
    {
        $areas = Area::where('active', true)->get();
        $categories = Category::where('active', true)->get();
        $requestType = RequestType::where('slug', 'mockup-impresso')->first();

        return view('tickets.create-mockup-impresso', compact('areas', 'categories', 'requestType'));
    }
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    ...
                    'description' => 'Categoria para solicitações de mock up impresso',
                    'active' => true,
                ]);
            }

            $slaData = $slaService->calculateSlaForTicket('mockup-impresso', 'medium', $category->id);
            ...
            if ($request->input('embalagem_transporte')) {
                $fullDescription .= "Embalagem para Transporte: " . Sanitizer::sanitize($request->input('embalagem_transporte')) . "\n\n";
            }
            ...
            $ticket = Ticket::create([
                ...
                'title' => 'Mock up Impresso - ' . Sanitizer::sanitize($request->input('titulo')),
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Solicitação de Mock up Impresso criada com sucesso!');
    }
