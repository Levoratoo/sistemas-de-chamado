...
class TicketGabaritoController extends Controller
{
    public function create(Request $request)
    {
        ...
        $requestType = RequestType::where('slug', 'gabarito')->first();
        
        return view('tickets.create-gabarito', compact('areas', 'categories', 'requestType'));
    }
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        $request->validate([
            ...
            'solicitacao_pre' => 'required|array|min:1',
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    'name' => 'Gabarito',
                    'description' => 'Categoria para solicitações de gabarito',
                    'active' => true,
                ]);
            }

            // Calcular SLA
            $slaData = $slaService->calculateSlaForTicket('gabarito', 'medium', $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'title' => 'Gabarito - ' . Sanitizer::sanitize($request->input('titulo')),
            ...
            TicketEventService::log(
                ...
                [
                    ...
                    'solicitacao_pre' => implode(', ', $solicitacoesPre),
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'Solicitação de Gabarito criada com sucesso!');
    }
