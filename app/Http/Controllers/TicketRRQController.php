...
class TicketRRQController extends Controller
{
    public function create(Request $request)
    {
        ...
        $requestType = RequestType::where('slug', 'rrq-reclamacao-qualidade')->first();
        
        return view('tickets.create-rrq', compact('areas', 'categories', 'requestType'));
    }
    ...
    public function store(Request $request, SlaCalculationService $slaService, NotificationService $notificationService)
    {
        ...
        $ticket = DB::transaction(function () use ($request, $user, $slaService, $notificationService) {
            ...
            if (!$category) {
                $category = Category::create([
                    'name' => 'RRQ - Reclamação de Qualidade',
                    'description' => 'Categoria para reclamações de qualidade',
                    'active' => true,
                ]);
            }
            ...
            $slaData = $slaService->calculateSlaForTicket('rrq-reclamacao-qualidade', $priorityLevel, $category->id);
            ...
            $ticket = Ticket::create([
                ...
                'title' => 'RRQ - ' . Sanitizer::sanitize($request->input('resumo')),
                ...
                'request_type' => 'rrq-reclamacao-qualidade',
        ...
        return redirect()->route('tickets.show', $ticket)
            ->with('success', 'RRQ criado com sucesso!');
    }
