...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-2xl p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Solicitação de Adiantamento à Fornecedores</h1>
                <p class="mt-2 text-sm text-gray-600">Preencha os dados abaixo para solicitar adiantamento para o financeiro.</p>
            </div>

            <form action="{{ route('tickets.store-advance') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf

                <!-- Abrir em nome de -->
                <div>
                    <label for="opened_on_behalf_of" class="block text-sm font-medium text-gray-700">Abrir esta requisição em nome de</label>
                    <select
                        id="opened_on_behalf_of"
                    ...
                    >
                        ...
                        @foreach(\App\Models\User::where('id', '!=', auth()->id())->orderBy('name')->get() as $user)
                            <option value="{{ $user->id }}" {{ old('opened_on_behalf_of') == $user->id ? 'selected' : '' }}>{{ $user->name }} - {{ $user->email }}</option>
                        @endforeach
                    </select>
                    @error('opened_on_behalf_of')
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('title') }}"
                        class="mt-1 w-full rounded-lg border-gray-200 px-4 py-2 text-gray-700 shadow-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500"
                        placeholder="Ex: Adiantamento para fornecedor ABC"
                    >
                    @error('title')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="4"
                        class="mt-1 w-full rounded-lg border-gray-200 px-4 py-2 text-gray-700 shadow-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500"
                        placeholder="Descreva a justificativa para o adiantamento..."
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <!-- Valor a ser pago -->
                    <div>
                        <label for="payment_amount" class="block text-sm font-medium text-gray-700">Valor a ser pago (opcional)</label>
                        <input
                            type="number"
                            id="payment_amount"
                            name="payment_amount"
                    ...
                    <!-- Data de pagamento -->
                    <div>
                        <label for="payment_date" class="block text-sm font-medium text-gray-700">Data de pagamento (opcional)</label>
                        <input
                            type="date"
                            id="payment_date"
                            name="payment_date"
