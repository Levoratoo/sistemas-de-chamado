...
<div class="py-12">
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg">
            <form method="POST" action="{{ route('tickets.store-benefits') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="justificativa" 
                              ...
                              placeholder="Descreva resumidamente o motivo da alteração do benefício."
                              required>{{ old('justificativa') }}</textarea>
                    <p class="mt-1 text-xs text-gray-500">
                        Descreva resumidamente o motivo da alteração do benefício.
                    </p>
                    @error('justificativa')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Tipo de Benefício -->
                    <div>
                        <label for="tipo_beneficio" class="block text-sm font-medium text-gray-700 mb-2">
                            Tipo de Benefício <span class="text-red-500">*</span>
                        </label>
                        <div class="relative">
                            <select id="tipo_beneficio" 
                                ...
                                <option value="vale-refeicao" {{ old('tipo_beneficio') == 'vale-refeicao' ? 'selected' : '' }}>Vale refeição</option>
                                <option value="vale-transporte" {{ old('tipo_beneficio') == 'vale-transporte' ? 'selected' : '' }}>Vale transporte</option>
                                <option value="kit-bebe-menino" {{ old('tipo_beneficio') == 'kit-bebe-menino' ? 'selected' : '' }}>Kit Bebê - MENINO</option>
                                <option value="kit-bebe-menina" {{ old('tipo_beneficio') == 'kit-bebe-menina' ? 'selected' : '' }}>Kit Bebê - MENINA</option>
                                <option value="multibeneficios" {{ old('tipo_beneficio') == 'multibeneficios' ? 'selected' : '' }}>Multibenefícios</option>
                                <option value="emprestimo-consignado" {{ old('tipo_beneficio') == 'emprestimo-consignado' ? 'selected' : '' }}>Empréstimo Consignado</option>
                                <option value="plano-saude" {{ old('tipo_beneficio') == 'plano-saude' ? 'selected' : '' }}>Plano de Saúde</option>
                            </select>
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            Assinale o tipo de benefício a ser alterado.
                        </p>
                    ...
                    <div>
                        <label for="data_alteracao" class="block text-sm font-medium text-gray-700 mb-2">
                            Data da Alteração do Benefício
                        </label>
                        <div class="relative">
                            <input type="date" 
                                   id="data_alteracao" 
...
document.addEventListener('DOMContentLoaded', function() {
    // Busca no select de Tipo de Benefício
    const tipoBeneficioSelect = document.getElementById('tipo_beneficio');
    if (tipoBeneficioSelect) {
        const maxSize = Math.min(tipoBeneficioSelect.options.length, 10);
        
        tipoBeneficioSelect.addEventListener('focus', function() {
            this.size = maxSize;
        });

        tipoBeneficioSelect.addEventListener('blur', function() {
            setTimeout(() => {
                this.size = 1;
            }, 200);
        });
        ...
        tipoBeneficioSelect.addEventListener('keypress', function(e) {
