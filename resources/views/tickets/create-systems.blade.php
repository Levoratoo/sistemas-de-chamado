...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg">
            <form method="POST" action="{{ route('tickets.store-systems') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           placeholder="Defina um resumo do problema que está ocorrendo"
                           required>
                    @error('titulo')
                ...
                <div>
                    ...
                    <textarea id="descricao" 
                              ...
                              rows="6"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Insira detalhes sobre o problema identificado"
                ...
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            Insira anexos de prints de tela, pdf ou arquivos que referenciem o problema que você está tendo.
                        </p>
                    </div>
                    @error('attachments')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Sistemas -->
                    <div>
                        <label for="sistema" class="block text-sm font-medium text-gray-700 mb-2">
                            Sistemas
                        </label>
                        <select id="sistema" 
                                name="sistema"
                            ...
                            <option value="metrics" {{ old('sistema') == 'metrics' ? 'selected' : '' }}>Metrics</option>
                            <option value="mitra" {{ old('sistema') == 'mitra' ? 'selected' : '' }}>Mitra</option>
                            <option value="wk-radar" {{ old('sistema') == 'wk-radar' ? 'selected' : '' }}>WK Radar</option>
                            <option value="wacesso" {{ old('sistema') == 'wacesso' ? 'selected' : '' }}>WAcesso</option>
                            <option value="sistemas-weisul" {{ old('sistema') == 'sistemas-weisul' ? 'selected' : '' }}>Sistemas Weisul</option>
                            <option value="office-365" {{ old('sistema') == 'office-365' ? 'selected' : '' }}>Office 365</option>
                            <option value="windows" {{ old('sistema') == 'windows' ? 'selected' : '' }}>Windows</option>
                            <option value="heidelberg" {{ old('sistema') == 'heidelberg' ? 'selected' : '' }}>Heidelberg</option>
                            <option value="esko-adobe-corel" {{ old('sistema') == 'esko-adobe-corel' ? 'selected' : '' }}>Esko / Adobe / Corel</option>
                            <option value="outros" {{ old('sistema') == 'outros' ? 'selected' : '' }}>Outros</option>
                        </select>
                        <p class="mt-1 text-xs text-gray-500">
                            Escolha uma opção de sistema no qual você precisa de ajuda.
                        </p>
                    </div>
