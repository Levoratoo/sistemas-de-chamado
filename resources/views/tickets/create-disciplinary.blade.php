...
<div class="py-12">
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg">
            <form method="POST" action="{{ route('tickets.store-disciplinary') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="justificativa" 
                              ...
                              placeholder="Descreva resumidamente o motivo da medida disciplinar."
                              required>{{ old('justificativa') }}</textarea>
                    <p class="mt-1 text-xs text-gray-500">
                        Descreva resumidamente o motivo da medida disciplinar. (campo obrigatório)
                    </p>
                    @error('justificativa')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="tipo_medida_disciplinar" class="block text-sm font-medium text-gray-700 mb-2">
                            Tipo da Medida Disciplinar <span class="text-red-500">*</span>
                        </label>
                        <select id="tipo_medida_disciplinar" 
                                name="tipo_medida_disciplinar"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                required>
                            <option value="">Selecione o tipo de medida disciplinar...</option>
                            <option value="nenhum" {{ old('tipo_medida_disciplinar', 'nenhum') == 'nenhum' ? 'selected' : '' }}>Nenhum</option>
                            <option value="advertencia-verbal" {{ old('tipo_medida_disciplinar') == 'advertencia-verbal' ? 'selected' : '' }}>Advertência verbal</option>
                            <option value="advertencia-escrita" {{ old('tipo_medida_disciplinar') == 'advertencia-escrita' ? 'selected' : '' }}>Advertência escrita</option>
                            <option value="suspensao" {{ old('tipo_medida_disciplinar') == 'suspensao' ? 'selected' : '' }}>Suspensão</option>
                        </select>
                        <p class="mt-1 text-xs text-gray-500">
                            Assinale o tipo de medida disciplinar a ser aplicada.
                        </p>
                        @error('tipo_medida_disciplinar')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="data_aplicacao" class="block text-sm font-medium text-gray-700 mb-2">
                            Data da Aplicação da Medida Disciplinar
                        </label>
                        <div class="relative">
                            <input type="date" 
                                   id="data_aplicacao" 
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        RH enviará o documento para aplicar a medida disciplinar.
                    </p>
                    @error('attachments')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    // Busca no select de Tipo de Medida Disciplinar
    const tipoMedidaSelect = document.getElementById('tipo_medida_disciplinar');
    if (tipoMedidaSelect) {
        const maxSize = Math.min(tipoMedidaSelect.options.length, 10);
        
        tipoMedidaSelect.addEventListener('focus', function() {
            this.size = maxSize;
        });

        tipoMedidaSelect.addEventListener('blur', function() {
            setTimeout(() => {
                this.size = 1;
            }, 200);
        });

        tipoMedidaSelect.addEventListener('change', function() {
            this.size = 1;
            this.blur();
        });

        tipoMedidaSelect.addEventListener('keypress', function(e) {
