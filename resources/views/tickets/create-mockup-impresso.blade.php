    ...
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div class="flex-1">
                    <h1 class="text-3xl font-bold text-gray-900">{{ $requestType->name ?? 'Mock up Impresso' }}</h1>
                    <p class="mt-2 text-gray-600">{{ $requestType->description ?? 'Solicitações de Mock up impresso para pré‑impressão' }}</p>
                </div>
        ...
        <div class="bg-white shadow-sm sm:rounded-lg">
            <form method="POST" action="{{ route('tickets.store-mockup-impresso') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
